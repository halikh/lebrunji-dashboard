...
import { liveStatusSlugs, searchTerm, startOfLocalDay, type OrderStatus } from './orders';
...
describe('liveStatusSlugs', () => {
  it('is everything that still needs somebody', () => {
    expect(liveStatusSlugs(SEEDED)).toEqual(['ordered', 'confirmed', 'driverSent']);
  });
  ...
  it('is empty rather than wrong when nothing is loaded', () => {
    ...
    expect(liveStatusSlugs([{ id: '1', slug: 'cancelled', name: 'x', progress: null }])).toEqual([]);
  });
});

describe('startOfLocalDay', () => {
  it('is midnight in the browser, not UTC', () => {
    const afternoon = new Date(2026, 7, 30, 14, 32, 5);
    const start = startOfLocalDay(afternoon);
    expect(start.getHours()).toBe(0);
    expect(start.getMinutes()).toBe(0);
    expect(start.getDate()).toBe(30);
  });

  it('still calls 00:30 today', () => {
    // The case that matters for a late-closing shop: trade at half past
    // midnight belongs to the day that has just started, and a UTC boundary
    // would file it under whichever day UTC happened to be on.
    const justAfterMidnight = new Date(2026, 7, 30, 0, 30, 0);
    expect(startOfLocalDay(justAfterMidnight).getDate()).toBe(30);
  });
});
