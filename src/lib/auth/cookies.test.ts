...
describe('authCookieAttributes', () => {
  it('is always HttpOnly', () => {
    ...
    expect(authCookieAttributes({ remember: true, isSecure: true }).httpOnly).toBe(true);
    expect(authCookieAttributes({ remember: false, isSecure: false }).httpOnly).toBe(true);
  });

  it('gives a lifetime when the box is ticked', () => {
    expect(authCookieAttributes({ remember: true, isSecure: true }).maxAge).toBe(REMEMBER_MAX_AGE);
  });
  ...
  it('omits the lifetime entirely when it is cleared', () => {
    ...
    const attributes = authCookieAttributes({ remember: false, isSecure: true });
  ...
  it('is lax, not strict', () => {
    ...
    expect(authCookieAttributes({ remember: true, isSecure: true }).sameSite).toBe('lax');
  });
  ...
  it('is only Secure over https', () => {
    ...
    expect(authCookieAttributes({ remember: true, isSecure: false }).secure).toBe(false);
    expect(authCookieAttributes({ remember: true, isSecure: true }).secure).toBe(true);
  });
});
