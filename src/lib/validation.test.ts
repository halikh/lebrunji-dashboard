...
const message = (r: ReturnType<typeof validateSlug>) => (r.ok ? null : r.message);

describe('validateSlug', () => {
  it('accepts the shape every existing slug takes', () => {
    for (const slug of ['kibbeh', 'kibbeh-plate', 'store-aurora', 'a1', '2for1-tuesday']) {
      expect(validateSlug(slug).ok).toBe(true);
    }
  });

  it('rejects the shapes that would break an import join', () => {
    for (const slug of ['Kibbeh', 'kibbeh plate', '-kibbeh', 'kibbeh-', 'kibbeh--plate', '']) {
...
describe('validateLocalizedText', () => {
  ...
  it('accepts a value in every language', () => {
    expect(validateLocalizedText({ en: 'Kibbeh', ar: 'كبة' }, languages, TEXT.name).ok).toBe(true);
  });
  ...
  it('names the missing language rather than failing anonymously', () => {
    ...
    expect(message(validateLocalizedText({ en: 'Kibbeh' }, languages, TEXT.name))).toContain('ar');
  });

  it('treats whitespace as empty, because the constraint does too', () => {
    expect(validateLocalizedText({ en: 'Kibbeh', ar: '   ' }, languages, TEXT.name).ok).toBe(false);
  });

  it('requires nothing at all when the column is nullable', () => {
    expect(validateLocalizedText({}, languages, TEXT.tagline, { optional: true }).ok).toBe(true);
    // …but half of an optional value is still half a value.
    expect(
      validateLocalizedText({ en: 'Fresh' }, languages, TEXT.tagline, { optional: true }).ok,
    ).toBe(false);
  });

  it('reports which language is too long', () => {
    const over = { en: 'x'.repeat(TEXT.name + 1), ar: 'كبة' };
    expect(message(validateLocalizedText(over, languages, TEXT.name))).toContain('en');
  });
  ...
  it('follows the languages it is given, not a hardcoded pair', () => {
    ...
    expect(validateLocalizedText({ en: 'a', ar: 'b' }, three, TEXT.name).ok).toBe(false);
    expect(validateLocalizedText({ en: 'a', ar: 'b', ku: 'c' }, three, TEXT.name).ok).toBe(true);
  });
});
...
describe('sniffImageType', () => {
  ...
  it('is what validateImage trusts', () => {
    ...
    expect(validateImage({ bytes: 1000, type: sniffImageType(renamed) }).ok).toBe(false);
  });
});
...
describe('validateImage', () => {
  it('accepts a reasonable photograph', () => {
    expect(
      validateImage({ bytes: 400_000, type: 'image/jpeg', width: 1200, height: 800 }).ok,
    ).toBe(true);
  });

  it('rejects one too large to upload', () => {
    expect(validateImage({ bytes: 20_000_000, type: 'image/jpeg' }).ok).toBe(false);
  });

  it('rejects a thumbnail pretending to be artwork', () => {
    expect(validateImage({ bytes: 1000, type: 'image/png', width: 40, height: 40 }).ok).toBe(false);
  });
});

describe('firstFailure', () => {
  it('reports the first problem, so a form shows one message at a time', () => {
    const result = firstFailure([validateSlug('ok'), validatePrice(-1), validateSlug('Bad')]);
